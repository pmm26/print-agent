...
const visibleItems = (items: (typeof groups)[number]['items'], isLinux: boolean) => items.filter(item => item.to !== '/setup/pair' || isLinux)

function DesktopNav({ isLinux }: { isLinux: boolean }) {
  const location = useLocation(); const navigate = useNavigate()
  return <nav aria-label="Dashboard sections" className="hidden items-center gap-1 md:flex">{groups.map(group => {
    const items = visibleItems(group.items, isLinux); const active = items.some(item => location.pathname === item.to)
    ...
    </DropdownMenuTrigger><DropdownMenuContent className="w-48">{items.map(item => <DropdownMenuItem key={item.to} onClick={() => navigate(item.to)} className={cn(location.pathname === item.to && 'bg-accent')}>
      <item.icon />{item.label}
    </DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu>
  })}</nav>
}

function MobileNav({ isLinux }: { isLinux: boolean }) {
  ...
  return <Sheet><SheetTrigger render={<Button variant="outline" size="icon" className="md:hidden" />}><Menu /><span className="sr-only">Open navigation</span></SheetTrigger>
    <SheetContent side="left" className="w-72"><SheetHeader><SheetTitle>Print Agent</SheetTitle></SheetHeader><nav className="space-y-5 px-4">{groups.map(group => <div key={group.label}>
      <p className="mb-1 px-2 text-xs font-medium text-muted-foreground">{group.label}</p>{visibleItems(group.items, isLinux).map(item => <SheetClose key={item.to} render={<Link to={item.to} className={cn('flex items-center gap-2 rounded-lg px-2 py-2 text-sm', location.pathname === item.to ? 'bg-accent font-medium' : 'hover:bg-accent/60')} />}><item.icon className="size-4" />{item.label}</SheetClose>)}
...
export function AppShell() {
  ...
  const isLinux = status.data?.platform === 'linux'
  ...
  return <JobDialogProvider><ThemeSync /><div className="min-h-svh bg-background text-foreground">
    <header className="sticky top-0 z-40 border-b bg-background/90 backdrop-blur"><div className="mx-auto flex h-16 max-w-7xl items-center gap-4 px-4 sm:px-6">
      <MobileNav isLinux={isLinux} /><Link to="/operations/jobs" className="flex items-center gap-2 font-semibold"><span className="grid size-8 place-items-center rounded-lg bg-primary text-primary-foreground"><Printer className="size-4" /></span><span className="hidden sm:inline">Print Agent</span></Link>
      <DesktopNav isLinux={isLinux} /><div className="ml-auto"><StatusBadge value={state} /></div>
    </div></header>
